...
export function buildOrderEmail(input: OrderMailInput): MailPayload {
  ...
  const preview = `הזמנה נקלטה. העבירו ${money(input.totalCents, input.currency)} בביט לקמפיין ${input.title}.`;
  ...
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#f3efe8;padding:32px 16px;">
    <tr><td align="center">
      <table role="presentation" width="560" cellpadding="0" cellspacing="0" style="max-width:560px;background:#fffdf9;border:1px solid #e7e0d6;border-radius:16px;overflow:hidden;">
        ...
        <tr><td style="padding:0 28px 20px;font-size:15px;line-height:1.6;color:#57534e;">שלום ${escapeHtml(input.artistName)}, הזמנת הקמפיין התקבלה. העבירו ${money(input.totalCents, input.currency)} בביט ללייבל. אחרי שהתשלום יאושר הקמפיין עולה לאוויר.</td></tr>
        ...
        <tr><td style="padding:20px 28px 28px;font-size:13px;line-height:1.6;color:#57534e;">
          העבירו את הסכום בביט. אחרי שהסטודיו מאשר שהכסף הגיע, הקמפיין עולה לאוויר והביצועים יופיעו אצלך באפליקציה.
